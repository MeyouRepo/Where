...
@UIState({LAYOUT_IN_SYSTEM_BARS, LIGHT_SYSTEM_BARS, View.SYSTEM_UI_FLAG_LOW_PROFILE})
public class Home extends Focus implements FunTouchListener {
  ...
  private void animation(boolean isStart) {
    ...
    if (isStart) {
      findViewById(R.id.homeTopToolBarRight)
          .startAnimation(
              AnimationUtils.loadAnimation(
                  getApplicationContext(), R.anim.home_top_toolbar_right_move_left));
      findViewById(R.id.imageButton)
          .startAnimation(
              AnimationUtils.loadAnimation(
                  getApplicationContext(), R.anim.home_imagebutton_move_bottom));
    }
  }

  private void clickHide() {
    findViewById(R.id.homeTopRightScrollShow)
        .startAnimation(loadAnimation(R.anim.button_click_scrollview_hide));
    ...
    findViewById(R.id.homeTopToolBarRight)
        .startAnimation(loadAnimation(R.anim.home_top_toolbar_right_move_left));
    findViewById(R.id.homeTopToolBarRight).setVisibility(View.VISIBLE);
  }
  ...
  private void topRightLoad() {
    for (int i = 1; i <= 7; i++) {
      final int z = i;
      findViewById(getResources().getIdentifier("tool_" + i, "id", getPackageName()))
          .setOnClickListener(
              v -> {
                homeLoad(z, true);
                clickHide();
      ...
      ((TextView) findViewById(getId("t" + i)))
          .setText(getStringArray(R.array.homeScrollViewTopRightLoad)[i - 1]);
      ((ImageView) findViewById(getId("i" + i))).setImageResource(0);
      if (i == getIntShare("HOME_CHECK_LOAD", 1)) {
        ((ImageView) findViewById(getId("i" + i)))
            .setImageResource(R.drawable.home_check_load_icon);
      }
    }
  }
  ...
  private void homeLoad(final int checkLoad, final boolean checkAnimation) {
    ...
    findViewById(R.id.homeTopRightToolBarTextView)
        .startAnimation(loadAnimation(R.anim.home_top_toolbar_right_text_fade_in));
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    ConstraintLayout homeTopToolbar = getDecorView().findViewById(R.id.home_top_toolbar);
    int paddingVertical = (int) ((getStatusBarHeight() * 0.618)) / 2;
    homeTopToolbar.setPadding(
        dpToPixels((float) 54 / 6),
        isPortrait() ? getStatusBarHeight() : paddingVertical,
        0,
        paddingVertical);
    runIn();
    ...
    if (!hasPermission(READ_EXTERNAL_STORAGE)
      ...
      requestPermissions(
          new String[] {
            READ_EXTERNAL_STORAGE, WRITE_EXTERNAL_STORAGE, READ_PHONE_STATE, CHANGE_WIFI_STATE
          },
          REQUEST_PERMISSIONS);
  }
  ...
  protected void onUIState() {
    ...
    // Toast.makeText(this, System.getProperty("os.name") + " " + System.getProperty("os.arch"),
                ...
                get(
                    ...
                    new CallBackListener() {
                      ...
                      public void onCallBacking(
                          HttpURLConnection httpURLConnection, byte[] bytes, boolean responseOver) {
                        if (!responseOver) response += new String(bytes);
                        else {
                                      ...
                                      startDownload(
                                          ...
                                          new DownloadListener() {
                                            ...
                                            public void onDownloading(
                                                String fileName, byte[] bytes1, String percentage) {
                                              sendNotification(
                                                  133,
                                            ...
                                            @Override
                                            public void onPause(
                                                String fileName, String percentage) {
  ...
  public void homeListToolBarClick(final View v) {
      ...
      if (v.getId() == getId("tool_a" + (i + 1))) {
        ...
        startActivity(
            getIntent(One.class)
                ...
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_A" + (i + 1)))).getText()));
      ...
      if (v.getId() == getId("tool_b" + (i + 1))) {
        startActivity(
            getIntent(Two.class)
                ...
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_B" + (i + 1)))).getText()));
        overridePendingTransition(R.anim.next_activity_in, R.anim.home_to_next_hide);
      }
    }
    ...
    for (int i = 0; i < 14; i++) {
      if (v.getId() == getId("tool_c" + (i + 1))) {
        startActivity(
            getIntent(Three.class)
                .putExtra(Three.class.getSimpleName(), (i + 1))
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_C" + (i + 1)))).getText()));
        overridePendingTransition(R.anim.next_activity_in, R.anim.home_to_next_hide);
      }
    }
    ...
    for (int i = 0; i < 8; i++) {
      if (v.getId() == getId("tool_d" + (i + 1))) {
        startActivity(
            getIntent(Four.class)
                .putExtra(Four.class.getSimpleName(), (i + 1))
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_D" + (i + 1)))).getText()));
        overridePendingTransition(R.anim.next_activity_in, R.anim.home_to_next_hide);
      }
    }
    ...
    for (int i = 0; i < 10; i++) {
      if (v.getId() == getId("tool_e" + (i + 1))) {
        startActivity(
            getIntent(Five.class)
                .putExtra(Five.class.getSimpleName(), (i + 1))
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_E" + (i + 1)))).getText()));
        overridePendingTransition(R.anim.next_activity_in, R.anim.home_to_next_hide);
      }
    }
      ...
      if (v.getId() == getId("tool_f" + (i + 1))) {
        startActivity(
            getIntent(Six.class)
                ...
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_F" + (i + 1)))).getText()));
        overridePendingTransition(R.anim.next_activity_in, R.anim.home_to_next_hide);
      }
    }
    ...
    for (int i = 0; i < 6; i++) {
      if (v.getId() == getId("tool_g" + (i + 1))) {
        startActivity(
            getIntent(Seven.class)
                .putExtra(Seven.class.getSimpleName(), (i + 1))
                .putExtra(
                    HOME_TO_NEXT_TITLE,
                    ((TextView) findViewById(getId("homeToNextTitle_G" + (i + 1)))).getText()));
        overridePendingTransition(R.anim.next_activity_in, R.anim.home_to_next_hide);
      }
    }
  }
  ...
  public void onActionDown(View v, MotionEvent event) {
    findViewById(R.id.homeTopToolBarRight)
        .setBackgroundResource(R.drawable.home_top_toolbar_right_drawable_pressed);
  ...
  public void onActionUp(View v, MotionEvent event) {
    findViewById(R.id.homeTopToolBarRight)
        .setBackgroundResource(R.drawable.home_top_toolbar_right_drawable);
  ...
  public void onClick(View v) {
    ...
    findViewById(R.id.homeSeeRe)
        .startAnimation(loadAnimation(R.anim.home_top_toolbar_right_text_fade_in));

    setViewRadius(15, R.id.homeTopRightScrollShow);
    ScrollView homeTopRightScrollShow = findViewById(R.id.homeTopRightScrollShow);
    RelativeLayout.LayoutParams homeTopRightScrollShowLayoutParams =
        (RelativeLayout.LayoutParams) homeTopRightScrollShow.getLayoutParams();
    int statusBarHeight = getStatusBarHeight();
    homeTopRightScrollShowLayoutParams.setMargins(
        0, statusBarHeight, (int) (statusBarHeight * 0.618), 0);
    homeTopRightScrollShow.setVisibility(View.VISIBLE);
    homeTopRightScrollShow.startAnimation(loadAnimation(R.anim.button_click_scrollview_show));
