...
public final class Dialog extends android.app.Dialog {
  ...
  private static final int COLOR_NIGHT = getGray(52);
  ...
  private void onUIState() {
    Objects.requireNonNull(getWindow()).setDimAmount(0.32f);
  ...
  public void show() {
    ...
    ScaleAnimation scaleAnimation =
        new ScaleAnimation(0.0f, 1.0f, 0.0f, 1.0f, RELATIVE_TO_SELF, 0.5f, RELATIVE_TO_SELF, 0.5f);
  ...
  public Dialog setMessage(String message) {
    if (message.length() > 14) message = message.substring(0, 13) + "...";
  ...
  private final class View extends android.view.View implements android.view.View.OnTouchListener {
    ...
    private View() {
      ...
      if (isNight()) textRightColor = getWhite(220);
      else textRightColor = BLACK;
    }
    ...
    protected void onDraw(Canvas canvas) {
      ...
      if (isNight()) paint.setColor(COLOR_NIGHT);
      else paint.setARGB(245, 255, 255, 255);

      canvas.drawRoundRect(main, radius, radius, paint);

      if (isNight()) paint.setColor(getWhite(220));
      ...
      canvas.drawText(
          Message, middle, main.top + (textSizeOnMessage / 2) + ((line - main.top) / 2), paint);
      ...
      if (isNight()) paint.setColor(getWhite(120));
      else paint.setColor(highLightColor);
      ...
      canvas.drawText(
          textOnLeft,
          main.left + (middle - main.left) / 2,
          line + ((main.bottom - line) / 2) + (textSizeOnButton / 2),
          paint);
      ...
      if (X < view.main.left || X > view.main.right || Y < view.main.top || Y > view.main.bottom) {
        if (event.getAction() == ACTION_DOWN) {
          ...
          if (currentTimeMillis() - touchDown <= 500 && canceledOnTouchOutside) {
            if (cancelOnTouchOutSideListener != null) cancelOnTouchOutSideListener.onClick(v);
            dismiss();
          }
        }
      }
